...
if [ "$NEEDS_INSTALL" = true ]; then
    ...
    filePath=$(node -e 'const os = require("os"); const path = require("path"); console.log(path.join(os.homedir(), ".claude.json"));')
    rm -f "$filePath"
    echo "🔄 Environment cleaned. Proceeding with installation...｜环境清理完成，开始安装..."
...
api_key=""
if [ -f "$rc_file" ] && grep -E -q 'export[[:space:]]+ANTHROPIC_BASE_URL=["'\'']?https://api\-inference\.modelscope\.cn/?["'\'']?' "$rc_file"; then
    echo ""
...
if [ -z "$api_key" ]; then
    echo ""
    echo "🔑 Please enter your Modelscope API Key｜设置你的 Modelscope API Key:"
    echo "   You can get your API Key from｜可访问右边地址获取 API Key: https://modelscope.cn/my/myaccesstoken"
...
echo "export ANTHROPIC_BASE_URL=https://api-inference.modelscope.cn/" >> "$rc_file"
